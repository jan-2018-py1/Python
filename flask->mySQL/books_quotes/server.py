...
mysql = MySQLConnector(app,'books_quotes')
...
# route to add quote
@app.route('/add_quote/<book_id>', methods=['get'])
def add_quote(book_id):
    return render_template("add_quote.html", book=book_id)
...
def process():
    ...
    if request.form['action'] == "Quote":
        return redirect('/add_quote/' + request.form['book-id'])
